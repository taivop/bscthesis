...
In addition to the non-changing components mentioned in the previous section, the extent to which presynaptic spikes influence postsynaptic voltage is modelled by the synaptic weights $\boldsymbol{g}=(g_i)$, with $i$ denoting the number of the synapse. The modification of these weights is the basis of learning in this model: increasing $g_i$ will increase the effect spikes at the $i$-th synapse have on postsynaptic voltage, and vice versa.
...
The model is implemented in MATLAB with source code freely available on Github\footnote{https://github.com/taivop/neuron}.
