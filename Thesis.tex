...
There are two kinds of synapses: input arriving at \emph{excitatory} synapses tend to increase the output of the postsynaptic neuron, input to \emph{inhibitory} synapses tends to make it fire less.
...
Output of a neuron takes the form of \emph{action potentials} or \emph{spikes}, rapid increases in membrane voltage followed by a return to the baseline voltage.
...
Inputs to the model are simulated as $N$ spike trains, one for each synapse, with a given mean firing rate $r$ specified independently for each synapse. The spike trains are Poissonian, i.e. the occurrence of a spike is independent of the time since the last spike. Additionally, correlation between inputs is simulated by specifying the correlation parameter $0 \leq c \leq 1$, and using code from \cite{macke2009} to generate input with the desired rate and correlation. The release of neurotransmitters caused by each presynaptic spike is assumed to last 1 ms.
...
Timestep, other important parameters?
...
\section{Task 1. Show that our model does PCA/ICA}
